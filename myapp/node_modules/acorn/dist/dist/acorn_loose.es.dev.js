...
LooseParser.prototype.indentationAfter = function indentationAfter(pos) {
  ...
  for (var count = 0;; ++pos) {
    ...
    if (ch === 32) ++count;else if (ch === 9) count += this$1.options.tabSize;else return count;
  }
};

LooseParser.prototype.closes = function closes(closeTok, indent, line, blockHeuristic) {
  if (this.tok.type === closeTok || this.tok.type === _acorn.tokTypes.eof) return true;
    ...
    try {
      ...
      if (/unterminated/i.test(msg)) {
        ...
        while (pos < this.input.length) {
          var ch = this$1.input.charCodeAt(pos++);
          if (ch === 34 || ch === 39 || (0, _acorn.isNewLine)(ch)) break;
        }
      ...
      if (replace) {
        if (this$1.options.locations) replace.loc = new _acorn.SourceLocation(this$1.toks, (0, _acorn.getLineInfo)(this$1.input, replace.start), (0, _acorn.getLineInfo)(this$1.input, replace.end));
        return replace;
      }
    }
  }
...
lp$1.parseStatement = function () {
  ...
  switch (starttype) {
    ...
    case _acorn.tokTypes._for:
      ...
      if (this.tok.type === _acorn.tokTypes.semi) return this.parseFor(node, null);
      ...
      if (this.tok.type === _acorn.tokTypes._in || this.isContextual("of")) return this.parseForIn(node, this.toAssignable(init));
    ...
    case _acorn.tokTypes._return:
      this.next();
      if (this.eat(_acorn.tokTypes.semi) || this.canInsertSemicolon()) node.argument = null;else {
    ...
    case _acorn.tokTypes._switch:
      ...
      while (!this.closes(_acorn.tokTypes.braceR, blockIndent, line, true)) {
        if (this$1.tok.type === _acorn.tokTypes._case || this$1.tok.type === _acorn.tokTypes._default) {
          ...
          cur.consequent = [];
          this$1.next();
          if (isCase) cur.test = this$1.parseExpression();else cur.test = null;
    ...
    default:
      ...
      if (isDummy(expr)) {
        this.next();
        if (this.tok.type === _acorn.tokTypes.eof) return this.finishNode(node, "EmptyStatement");
...
lp$1.parseFor = function (node, init) {
  ...
  if (this.eat(_acorn.tokTypes.semi) && this.tok.type !== _acorn.tokTypes.semi) node.test = this.parseExpression();
  if (this.eat(_acorn.tokTypes.semi) && this.tok.type !== _acorn.tokTypes.parenR) node.update = this.parseExpression();
  this.popCx();
  this.expect(_acorn.tokTypes.parenR);
  node.body = this.parseStatement();
...
lp$1.parseClass = function (isStatement) {
  ...
  var node = this.startNode();
  this.next();
  if (isStatement == null) isStatement = this.tok.type === _acorn.tokTypes.name;
  if (this.tok.type === _acorn.tokTypes.name) node.id = this.parseIdent();else if (isStatement) node.id = this.dummyIdent();else node.id = null;
...
lp$1.parseFunction = function (node, isStatement, isAsync) {
  ...
  if (isStatement == null) isStatement = this.tok.type === _acorn.tokTypes.name;
  if (this.tok.type === _acorn.tokTypes.name) node.id = this.parseIdent();else if (isStatement) node.id = this.dummyIdent();
...
lp$2.parseSubscripts = function (base, start, noCalls, startIndent, line) {
  ...
  for (;;) {
    if (this$1.curLineStart != line && this$1.curIndent <= startIndent && this$1.tokenStartsLine()) {
      if (this$1.tok.type == _acorn.tokTypes.dot && this$1.curIndent == startIndent) --startIndent;else return base;
    }
    ...
    if (this$1.eat(_acorn.tokTypes.dot)) {
      ...
      if (maybeAsyncArrow && this$1.eat(_acorn.tokTypes.arrow)) return this$1.parseArrowExpression(this$1.startNodeAt(start), exprList, true);
      var node$2 = this$1.startNodeAt(start);
      node$2.callee = base;
  ...
  switch (this.tok.type) {
    ...
    case _acorn.tokTypes.name:
      ...
      if (id.name === "async" && !this.canInsertSemicolon()) {
        if (this.eat(_acorn.tokTypes._function)) return this.parseFunction(this.startNodeAt(start), false, true);
...
lp$2.parseObj = function () {
  ...
  while (!this.closes(_acorn.tokTypes.braceR, indent, line)) {
    ...
    if (!prop.computed && prop.key.type === "Identifier" && prop.key.name === "async" && this$1.tok.type !== _acorn.tokTypes.parenL && this$1.tok.type !== _acorn.tokTypes.colon && !this$1.canInsertSemicolon()) {
      this$1.parsePropertyName(prop);
      isAsync = true;
    } else {
      isAsync = false;
    ...
    } else if (this$1.options.ecmaVersion >= 5 && prop.key.type === "Identifier" && !prop.computed && (prop.key.name === "get" || prop.key.name === "set") && this$1.tok.type != _acorn.tokTypes.comma && this$1.tok.type != _acorn.tokTypes.braceR) {
...
lp$2.parsePropertyAccessor = function () {
  if (this.tok.type === _acorn.tokTypes.name || this.tok.type.keyword) return this.parseIdent();
};
...
lp$2.parseMethod = function (isGenerator, isAsync) {
  ...
  this.inAsync = node.async;
  node.params = this.parseFunctionParams();
  node.expression = this.options.ecmaVersion >= 6 && this.tok.type !== _acorn.tokTypes.braceL;
