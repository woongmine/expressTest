...
function importFrom(data, context) {
  ...
  for (; nextEnd < data.length;) {
    ...
    context.left.unshift([data.substring(nextEnd + 1), override(context, {
      shallow: false
    })]);
...
function commentScanner(data) {
  ...
  var lastEndIndex = 0;
  var noComments = false; // test whether an index is located within a comment
  ...
  return function scanner(idx) {
    ...
    var globalEndIndex = 0; // return if we know there are no more comments
    ...
    do {
      ...
      localEndIndex = localStartIndex + comment[0].length; // calculate the indexes relative to the full original data

      globalEndIndex = localEndIndex + lastEndIndex;
      globalStartIndex = globalEndIndex - comment[0].length; // chop off data up to and including current comment block
...
function inlineRemoteResource(importedFile, mediaQuery, context) {
  ...
  get(requestOptions, function (res) {
    ...
    res.on('end', function () {
      ...
      if (context.rebase) importedData = rewriteUrls(importedData, {
        toBase: originalUrl
      }, context);
