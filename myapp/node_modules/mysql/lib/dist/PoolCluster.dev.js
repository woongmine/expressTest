...
PoolCluster.prototype.add = function add(id, config) {
  ...
  var nodeId = _typeof(id) === 'object' ? 'CLUSTER::' + ++this._lastId : String(id);

  if (this._nodes[nodeId] !== undefined) {
    throw new Error('Node ID "' + nodeId + '" is already defined in PoolCluster.');
  }

  var poolConfig = _typeof(id) !== 'object' ? new PoolConfig(config) : new PoolConfig(id);
  this._nodes[nodeId] = {
    id: nodeId,
    errorCount: 0,
    pool: new Pool({
...
function isRegExp(val) {
  return _typeof(val) === 'object' && Object.prototype.toString.call(val) === '[object RegExp]';
}
