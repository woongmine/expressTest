...
function Lexer(str, options) {
  ...
  if (typeof str !== 'string') {
    throw new Error('Expected source code to be a string but got "' + _typeof(str) + '"');
  }
...
Lexer.prototype = {
  ...
  attrs: function attrs() {
    if ('(' == this.input.charAt(0)) {
      ...
      var isEndOfAttribute = function isEndOfAttribute(i) {
        ...
        if (loc === 'key') {
          if (whitespaceRe.test(str[i])) {
            ...
            for (var x = i; x < str.length; x++) {
              if (!whitespaceRe.test(str[x])) {
                // starts a `value`
                if (str[x] === '=' || str[x] === '!') return false; // will be handled when x === i
                else if (str[x] === ',') return false; // attribute ended
                  else return true;
              }
            }
          ...
          if (state.isNesting() || state.isString()) return false; // if the current value expression is not valid JavaScript, then
      ...
      for (var i = 0; i <= str.length; i++) {
        ...
        if (str[i] === '\n') {
          ...
          // the attribute.
          lineno++;
          this.colno = 1; // If the key has not been started, update this.lineno immediately.
